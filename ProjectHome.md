...
### RESTful ###
A small RESTful example showing howto create, get, update and delete book resources:
...
### Filters ###
An example showing the use of filters:
...
### Attributes ###
An example showing the use of attributes:
...
## Javadoc ##
As of now javadoc is not available online. However, if you download the source (see Source tab on howto) javadoc is very simple to generate with maven. Just navigate to the project folder (where pom.xml is situated) and do `mvn javadoc:javadoc`. The result is put in `/target/site/apidocs`